...
## 14.5 AgentFlow 信息流可视化组件

| 组件 | 文件 | 职责 |
|------|------|------|
| AgentFlowBroadcaster | pi/AgentFlowBroadcaster.java | 后端 WebSocket 广播器，将 Agent 任务分发/完成/失败事件广播到前端 |
| AgentFlowView | components/Agents/AgentFlowView.tsx | 前端 SVG 可视化组件，实时显示 Agent 节点、连接曲线、动态粒子动画、事件日志 |
| Flow Hook | gent/EnhancedOrchestratorAgent.java | 在 taskDispatch 和 taskResult 处插入广播钩子 |
| WebSocket消息类型 | gent_flow_event | dispatch (Orchestrator→Agent) / complete (Agent→Orchestrator) / agent_status |

**AgentFlow 工作原理：**
1. 当 EnhancedOrchestratorAgent 向子 Agent 分发任务时，触发 AgentFlowBroadcaster.broadcastDispatch()
2. 当子 Agent 任务完成/失败时，触发 AgentFlowBroadcaster.broadcastComplete()
3. TaskWebSocketServer 将消息广播到所有连接的前端客户端
4. AgentFlowView 通过 wsService.onMessage() 订阅 agent_flow_event，实时更新可视化图表
