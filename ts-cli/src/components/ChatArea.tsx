...
function shouldStartCollapsed(toolCalls: ToolCall[], index: number): boolean {
  ...
  // Last completed/failed tool call stays expanded
...
export const ChatArea = memo(function ChatArea({ messages, currentMessage, scrollOffset, terminalRows, reservedRows, terminalCols, toolCallsExpanded }: Props) {
  ...
  const { maxVisible, total, start, end, visibleMessages, isScrolledUp, scrollPercent } = useMemo(() => {
    ...
    return {
      maxVisible, total, start, end,
      visibleMessages: allMessages.slice(start, end),
      isScrolledUp: clampedOffset > 0,
      scrollPercent: total > 0 ? Math.round(((total - end) / total) * 100) : 0,
    };
...
function ToolCallDisplay({ tc, collapsed, onToggle }: { tc: ToolCall; collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <Box flexDirection="column" paddingLeft={1}>
      {/* Header — clickable to toggle args */}
      <Box>
      ...
      {/* Result — always visible, never truncated */}
      {tc.result && (
