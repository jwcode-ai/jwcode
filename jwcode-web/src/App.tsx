...
function App() {
  ...
  const handleNewSession = useCallback(() => {
    startTransition(() => {
      ...
      useExecutionModeStore.getState().setMode('act');
    });
  ...
  const sendMessage = useCallback(async (sessionId: string, content: string, referencedFiles?: string[]) => {
    ...
    const executionModeStore = useExecutionModeStore.getState();
    ...
    const currentMode = executionModeStore.mode;
    if (currentMode === 'plan') {
      wsService.send({ type: 'plan', sessionId, message: finalContent });
    } else if (currentMode === 'goal') {
      const runId = `wf-${Date.now()}-${crypto.randomUUID()}`;
      useChatStore.getState().startGeneration(sessionId);
      useWorkflowStore.getState().upsert(sessionId, {
        runId,
        status: 'RUNNING',
        completedPhases: 0,
        totalPhases: 0,
      ...
      wsService.send({
        ...
        projectId: workspaceDir,
        memoryEnabled: true,
        checkpointPolicy: 'phase-and-token',
      });
    } else {
      wsService.send({ type: 'chat', sessionId, message: finalContent });
    }
  }, [autoNameSession, workspaceDir]);

  const stopGeneration = useCallback((sessionId: string) => {
    const workflow = useWorkflowStore.getState().getForSession(sessionId);
    if (workflow && !['COMPLETED', 'CANCELLED', 'ERROR', 'FAILED'].includes(workflow.status)) {
      wsService.send({ type: 'workflow_cancel', sessionId, runId: workflow.runId });
    } else {
  ...
  const pauseGeneration = useCallback((sessionId: string) => {
    const workflow = useWorkflowStore.getState().getForSession(sessionId);
    if (workflow && ['RUNNING', 'RESUMING'].includes(workflow.status)) {
      wsService.send({ type: 'workflow_pause', sessionId, runId: workflow.runId });
    } else {
  ...
  const resumeGeneration = useCallback((sessionId: string) => {
    ...
    if (workflow && workflow.status === 'PAUSED') {
      wsService.send({ type: 'workflow_resume', sessionId, runId: workflow.runId });
    } else {
  ...
  // TaskListPanel extracted to components/TaskList/TaskListPanel.tsx
